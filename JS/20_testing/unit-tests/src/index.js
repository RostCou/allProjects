...
export function validationNumberCard(cardNumber) {
  const numberValid = valid.number(cardNumber);
  return numberValid.isValid;
}

export function validationCvvCard(cardCvv) {
  return valid.cvv(cardCvv).isValid && cardCvv.length === 3;
}
  ...
  switch (id) {
    case "card-number":
      return {
        ...
        onblur() {
          if (validationNumberCard(this.value)) userCard.cardNumberValid = true;
    ...
    case "card-code":
      return {
        ...
        ...{
          onblur() {
            if (validationCvvCard(this.value)) {
...
const inputCardNumber = el(
  "input.form-control",
  configurationCard("card-number")
);
const inputCardDate = el("input.form-control", configurationCard("card-date"));
const inputCardCode = el("input.form-control", configurationCard("card-code"));
const formButton = el(
...
export function createForm() {
  return el("form", [
    el("input", { placeholder: "Номер карты" }),
    el("input", { placeholder: "ММ/ГГ" }),
    el("input", { placeholder: "CVV/CVC" }),
